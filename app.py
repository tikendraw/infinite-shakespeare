...
def load_model(itos):
    ...
    if os.path.isfile(Path(__file__).parent / "model_weights/gpt.pth"):
        try:
            gpt.load_state_dict(torch.load(Path(__file__).parent / "model_weights/gpt.pth") if device=='cuda' else torch.load(Path(__file__).parent / "model_weights/gpt.pth", map_location=torch.device('cpu')))
...
def main():
    ...
    with col1:
        input_string = st.text_input(' ', placeholder='Give Shakespeare something to start with...').strip()
    with col2:
        max_len = int(st.number_input('Predict tokens', min_value=100, value=500))
        
    with col12:
        temperature = float(st.number_input('Temperature', min_value=0.5, max_value=10.0, value=1.0, step=0.1))
    ...
    if generate and input_string:
        ...
        out = gpt.write(x.to(config.device), max_new_tokens=max_len, temperature=temperature)
